...
namespace skr::hotswap
{
    ...
    bool FileWatcher::AddWatch(const fs::path& directoryPath)
    {
        if (m_NotifyFd == -1)
        {
            ...
            {
                log::Error()
                    << SKR_HOTSWAP_LOG_PREFIX << "Failed inotify_init call."
            ...
            if (flags == -1)
            {
                log::Error() << SKR_HOTSWAP_LOG_PREFIX
                             << "Failed to get flags from directory watch fd. "
                             << log::LastOsError() << log::End();
                return false;
            }
            ...
            if (fcntl(m_NotifyFd, F_SETFL, flags | O_NONBLOCK) == -1)
            {
                log::Error()
                    ...
                    << "Failed to set directory watch fd to nonblocking. "
                    << log::LastOsError() << log::End();
                return false;
            }
        }
    ...
    bool FileWatcher::RemoveWatch(const fs::path& directoryPath)
    {
        auto watchIt = std::find_if(
            m_DirectoryPathsByWd.begin(), m_DirectoryPathsByWd.end(),
    ...
    void FileWatcher::PollChanges()
    {
        ...
        if (ret > 0)
        {
            ...
            {
                ssize_t nBytes = read(fds[i].fd, m_NotifyBuffer.data(),
                                      m_NotifyBuffer.size());
                if (nBytes <= 0)
                {
                    log::Error() << SKR_HOTSWAP_LOG_PREFIX
                                 << "Failed to read notify fd. "
                                 << log::LastOsError() << log::End();
                    return;
                }

                for (char* pData = m_NotifyBuffer.data();
                     pData < m_NotifyBuffer.data() + nBytes;)
                {
                    struct inotify_event* pNotifyEvent =
                        reinterpret_cast<inotify_event*>(pData);
    ...
    void FileWatcher::HandleNotifyEvent(struct inotify_event* pNotifyEvent)
    {
        ...
        if (pNotifyEvent->mask & IN_CREATE ||
            ...
            pNotifyEvent->mask & IN_MODIFY || pNotifyEvent->mask & IN_DELETE ||
    ...
    void FileWatcher::CloseWatch(int wd)
    {
        if (inotify_rm_watch(m_NotifyFd, wd) == -1)
        {
            log::Warning() << SKR_HOTSWAP_LOG_PREFIX
                           << "Failed to remove watch from inotify. "
                           << log::LastOsError() << log::End();
        }
    }
} // namespace skr::hotswap
